...
namespace Proyecto_LaboratorioJARAF.Repository
{
    public class ClsRCita : ICitas
    {
        ...
        {
            using (LABORATORIO_CLINICO_JARAF2Entities conexionBd = new LABORATORIO_CLINICO_JARAF2Entities())
            {
                cita = conexionBd.CITAS.Find(cita.IdCita);
                conexionBd.CITAS.Remove(cita);
